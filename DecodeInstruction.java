...
public class DecodeInstruction {
	...
	public static String[] InstDecode(String instruction, String pc) {
		...
		Main.branch = Integer.parseInt(s[3]);
		...
		Main.jump = Integer.parseInt(s[14]);

		Main.decode = instruction + " in Decode Stage." + "\nread data 1: "
				+ Main.prepend(Integer.toBinaryString(Main.readdata1), 32) + "\nread data 2: "
				+ Main.prepend(Integer.toBinaryString(Main.readdata2), 32) + "\nsign-extend: " + s[11] + "\nNext PC: "
				+ fromDtoB(Integer.parseInt(pc) + 1) + "\nrt: " + s[12] + "\nrd: " + s[13] + "\nWB Controls: MemtoReg: "
				+ Main.memtoreg + " RegWrite: " + Main.regwrite + "\nMEM Controls: MemRead: " + Main.memread
				+ " MemWrite: " + Main.memwrite + " Branch: " + Main.branch + " Jump: " + Main.jump
				+ "\nEX Controls: RegDst: " + Main.regdst + " ALUOp: " + s[10] + " ALUSrc: " + Main.Alusrc + "\nPCsrc: "
				+ Main.pcsrc;
	...
	public static String[] ContUnit(String op) {
		switch (op) {
		// memRead,memWrite,RegWrite,branch,RegDst,MemtoReg,AluSrc
		case "0000":
		case "0001":
		case "0010":
		case "0100":
			String[] a = { "0", "0", "1", "0", "1", "0", "0", "0" };
			return a;
		case "0011":
		case "0101":
		case "1100":
		case "0110":
		case "0111":
			String[] b = { "0", "0", "1", "0", "1", "0", "1", "0" };
			return b;
		case "1000":
			String[] c = { "1", "0", "1", "0", "1", "1", "1", "0" };
			return c;
		case "1001":
			String[] d = { "0", "1", "0", "0", "0", "0", "1", "0" };
			return d;
		case "1010":
		case "1011":
			String[] e = { "0", "0", "0", "1", "0", "0", "0", "0" };
			return e;
		case "1101":
			String[] f = { "0", "0", "0", "0", "0", "0", "0", "1" };
			return f;
		}
		return null;
	}
